...
## 3.2 多变量线性回归 Linear Regression with Multiple Variables
...
### 3.2.3 梯度下降法实践1-特征缩放 Gradient Descent in Practice I - Feature Scaling
...
💬 以房价预测问题为例，假设我们拥有两 个特征：拥有的土地的宽度 frontage 以及 房子的深度 depth
...
![image-20201022145205841](https://gitee.com/wugenqiang/images/raw/master/02/image-20201022145205841.png)
...
![solve](https://gitee.com/wugenqiang/images/raw/master/02/image-20201022150047353.png)
...
def normalEqn(X, y):
    # X.T@X等价于X.T.dot(X)
    theta = np.linalg.inv(X.T@X)@X.T@y  # n.linalg.inv 求逆
    return theta
```
...
随着我们要讲的学习算法越来越复杂，例如，当我们讲到分类算法，像逻辑回归算法，我们会看到，实际上对于那些算法，并不能使用标准方程法。对于那些更复杂的学习算法，我们将不得不仍然使用梯度下降法。因此，**梯度下降法是一个非常有用的算法**，可以用在`有大量特征变量的线性回归问题`。或者我们以后在课程中，会讲到的一些其他的算法，因为正规方程法不适合或者不能用在它们上。但对于这个特定的线性回归模型，正规方程法是一个比梯度下降法更快的替代算法。所以，根据具体的问题，以及你的特征变量的数量，这两种算法都是值得学习的。
