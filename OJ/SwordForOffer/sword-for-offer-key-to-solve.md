...
> [牛客网 - 剑指 Offer 编程网站](https://www.nowcoder.com/ta/coding-interviews)
>
> 或者 [LeetCode - 剑指 Offer](https://leetcode-cn.com/problemset/lcof/)
...
以上是修改数组元素值找出重复的数字，能不能通过不修改数组找出重复的数字？答案是可以的：
...
public:
    ...
    TreeNode* reConstructBinaryTree(vector<int> pre, vector<int> vin) {
        int n = pre.size();//记录节点
        for (int i = 0; i < n; i++) {//中序序列每个元素对应的位置
            pos[vin[i]] = i;
        }
        return dfs(pre, vin, 0, n - 1, 0, n - 1);
    }
    TreeNode* dfs(vector<int> pre, vector<int> vin, int pl, int pr, int vl, int vr) {//深度优先
        //前序序列：根左右 pre[pl]
        //中序序列：左根右
        if (pl > pr) {
            return NULL;
...
[做题地址](https://www.nowcoder.com/practice/c6c7742f5ba7442aada113136ddea0c3?tpId=13&tqId=11160&tPage=1&rp=1&ru=/ta/coding-interviews&qru=/ta/coding-interviews/question-ranking)

### 题目描述

求斐波那契数列的第 n 项，n <= 39。

![image-20200419091035335](https://gitee.com/wugenqiang/PictureBed/raw/master/CS-Notes/20200419091043.png)
